...
void gauss_elimination(int n, double a[MAX_SIZE+1][MAX_SIZE+1], double b[MAX_SIZE+1], double X[MAX_SIZE+1]) {
    ...
    for (int i = 0; i < n-1; i++) {
        ...
        // Intercambiar filas si el pivote no está en la diagonal
        if (p != i) {
            for (int m = i; m < n; m++) {
                aux = a[p][m];
                a[p][m] = a[i][m];
    ...
    // Determinante (producto de la diagonal)
    det = 1.0;
    for (int i = 0; i < n; i++) det *= a[i][i];
    printf("------------------DETERMINANT------------------\n");
    printf("Determinant = %lf\n", det);
    if (fabs(det) < 1e-12) {
        printf("⚠️ Sistema singular, no hay solución única\n");
        exit(1);
    }
    ...
    for (int i = n-1; i >= 0; i--) {
        ...
        if (fabs(a[i][i]) < 1e-12) {
            printf("❌ División por cero en back substitution\n");
            exit(1);
        }
        X[i] = sum / a[i][i];
    }
