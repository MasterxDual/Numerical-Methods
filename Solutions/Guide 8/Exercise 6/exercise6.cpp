...
/* We did a calculation to obtain X(t) and Y(t) using trigonometric with the angles given in the data table of the exercise.
    We have the following data points to use in two data.txt. One for X(t) and t, and the other for Y(t) and t.
    t X(t) [meters]
    9 1401.92
    10 1450.50
    11 1498.64
    
    And the other data table:
    t Y(t) [meters]
    9 1987.35
    10 2000.85
    11 2013.51

    Now we want to use this equations: 
    Vx(t) = dX/dt and Vy(t) = dY/dt
    So we will use numerical differentiation to calculate the derivatives of order 1, with an error of O(h) and O(h²)
    Error O(h):
    t               f'(t) = Vx(t) [m/s]
    9.000000        48.580000
    10.000000       48.360000
    ...
    Notice that in t = 10 with an error of O(h²) we have bigger values, this is because error O(h²) needs more points to be more accurate.
    So we use O(h) because we have only 3 points and it's perfect.

    We only need the values in t = 10. So we use Vx = 48.36 m/s and Vy = 13.08 m/s

    We know that V = (Vx² + Vy²)^(1/2) = (48.36² + 13.08²)^(1/2) = 50.098 m/s
    And the angle of ascent is θ = arctan(Vy/Vx) = arctan(13.08/48.36) = 15.13°
            ...
            case 1: 
                if(function_data_table == 1) {
                    ...
                    // Read data points from file
                     if (!read_data_points("data2.txt", X, Y, &n)) {
