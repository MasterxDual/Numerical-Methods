...
int main(int argc, char const *argv[]) {
    ...
    // Weights that appears in this exercise. We don't define his values because it could be any value. So it's generic for demostration
    double Weight[MAX_POINTS];
    ...
    printf("1. Linear Case\n");
    printf("2. Polynomial Case\n");
    scanf("%d", &option);
    switch(option) {
        case 1:
            for(int l = 0; l <= 1; l++) {
                sumxy = 0.0;
                for(int i = 0; i < n; i++) {
                    sumxy += Y[i] * pow(X[i], l) * pow(Weight[i], 2);
                }
                b[l] = sumxy;
                for(int m = 0; m <= 1; m++) {
                    sumx = 0.0;
                    for(int i = 0; i < n; i++) {
                        sumx += pow(X[i], l + m) * pow(Weight[i], 2); 
                    }
                    A[l][m] = sumx;
                }
            }
        ...
        case 2:
            ...
            // The only difference with other exercises is the weights that appears in the sums
