...
int main(int argc, char const *argv[]) {
    ...
    switch(option) {
        ...
        case 3:
            ...
            printf("\nThe interpolated value of the specific heat at T = %.2f°C is: %.6f kJ/kg·K\n", X_hat, Pn);
            if (fx != 0.0) {
                printf("\nerror = |f(T) - Pn(T)| = %lf\n", error);
            } else {
...
double valuate_function(double T) {
    // For the problem of specific heat of aluminum vs. temperature
    // We don't have an exact theoretical function to compare with experimental data
    // The Lagrange polynomial IS the approximation we are looking for
    
    printf("Note: This problem uses experimental data without exact theoretical function\n");
    printf("Cannot calculate actual error - only analysis of approximation\n");

    return 0.0;  // No error is calculated in experimental problems
}