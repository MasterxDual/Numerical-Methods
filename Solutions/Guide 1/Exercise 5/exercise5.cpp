...
Using basic maths to solve the equation we finally get the following function:
    f(x) = ((log(x) - 1 + (1/x)) / (log(x) + (3 - (3 * (1 / x) )) / 2)) - 0.3
    We use this function in math_function(m) to find its root.
Bisection Method applied
    a = 5.3
    b = 5.5
    tolerance = 1e-3
    --> Aproximated root = 5.413281, absolute error = 0.000781 <--> 5.4133 +- 0.0008
...
Regula Falsi applied
    ...
    b = 5.5
    tolerance = 1e-3
    --> Aproximated root = 5.412548, absolute error = 0.000015 <--> 5.41255 +- 0.00002
...
double math_function(double x) {
    return ((log(x) - 1 + (1/x)) / (log(x) + (3 - (3 * (1 / x) )) / 2)) - 0.3; //This is the function from exercise 5
}
