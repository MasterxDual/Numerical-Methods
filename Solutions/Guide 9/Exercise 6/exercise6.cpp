...
We have the following exercise:
    f(t, y) = (0.01 * (70e-3 - y) * (50e-3 - y));
    y(t) = ((350 * (1 - exp(-0.2*x))) / (7 - 5*exp(-0.2*x)));
    h = 0.5 
    X0 = 0, Xf = 20
    y(0) = 0
    We need to use the same scale as the exact solution, in milimols/liter. So, we need to multiply by 1e3 the constants 70e-3 and 50e-3.
...
double f(double x, double y) {
    // return (0.01 * (70e-3 - y) * (50e-3 - y));
    return (0.01 * (70 - y) * (50 - y));
}

double y(double x) {
    return ((350 * (1 - exp(-0.2*x))) / (7 - 5*exp(-0.2*x)));
}
