...
/**
 * Function to define function f1(t, y1, y2, y3, y4)
 * @param t The independent variable
 * @param y1 The first dependent variable
 * @param y2 The second dependent variable
 * @param y3 The third dependent variable
 * @param y4 The fourth dependent variable
 * @return The value of the function at (t, y1, y2, y3, y4)
 */
double f14(double t, double y1, double y2, double y3, double y4);

/**
 * Function to define function f2(t, y1, y2, y3, y4)
 * @param t The independent variable
 * @param y1 The first dependent variable
 * @param y2 The second dependent variable
 * @param y3 The third dependent variable
 ...
 * @return The value of the function at (t, y1, y2, y3, y4)
 */
double f24(double t, double y1, double y2, double y3, double y4);


/**
 * Function to define function f3(t, y1, y2, y3, y4)
 * @param t The independent variable
 * @param y1 The first dependent variable
 * @param y2 The second dependent variable
 * @param y3 The third dependent variable
 ...
 * @return The value of the function at (t, y1, y2, y3, y4)
 */
double f34(double t, double y1, double y2, double y3, double y4);

/**
 * Function to define function f4(t, y1, y2, y3, y4)
 * @param t The independent variable
 * @param y1 The first dependent variable
 * @param y2 The second dependent variable
 * @param y3 The third dependent variable
 ...
 * @return The value of the function at (t, y1, y2, y3, y4)
 */
double f44(double t, double y1, double y2, double y3, double y4);
    ...
    switch(choice) {
        ...
        case 4:
            ...
            if(edo_count == 2) {
                ...
                // En la sección principal donde manejas 4 EDOs:
                X[0] = X0;
                printf("Insert initial data Y01 = Y1(X0):\n");
                scanf("%lf", &Y1[0]);
                printf("Insert initial data Y02 = Y2(X0):\n");
                ...
                if (conv_choice == 1) {
                    convergence_factor_rk4_4(
                        ...
                        f14, f24, f34, f44,
                        "convergence_rk4_edo4_1.txt",
                        "convergence_rk4_edo4_2.txt", 
                        "convergence_rk4_edo4_3.txt",
                        "convergence_rk4_edo4_4.txt"
                    );
                }
                ...
                // ============================================================
                // == Prints of results (sin errores exactos)
                // ============================================================
                ...
                // == Save results and plot
                // ============================================================
                save_in_txt("circuito_q1.txt", X, Y1, n);
                save_in_txt("circuito_q2.txt", X, Y2, n);
                save_in_txt("circuito_i1.txt", X, Y3, n);
                save_in_txt("circuito_i2.txt", X, Y4, n);
                
                // Finalmente, graficar los resultados
...
// f3: di1/dt = (E - R*i1 - (q1 - q2)/C) / L
double f34(double t, double y1, double y2, double y3, double y4) {
    return (E - R*y3 - (y1 - y2)/C) / L;
}

// f4: di2/dt = (-R*i2 - (2*q2 - q1)/C) / L
double f44(double t, double y1, double y2, double y3, double y4) {
    return (-R*y4 - (2*y2 - y1)/C) / L;
}
...
void rk4_step4(double t, double *y1, double *y2, double *y3, double *y4, double h,
               double (*f1)(double, double, double, double, double),
               double (*f2)(double, double, double, double, double),
               double (*f3)(double, double, double, double, double),
               double (*f4)(double, double, double, double, double)) {
    ...
    k11 = f1(t, *y1, *y2, *y3, *y4);
    k12 = f2(t, *y1, *y2, *y3, *y4);
    k13 = f3(t, *y1, *y2, *y3, *y4);
    k14 = f4(t, *y1, *y2, *y3, *y4);
    
    k21 = f1(t + h/2.0, *y1 + h/2.0*k11, *y2 + h/2.0*k12, *y3 + h/2.0*k13, *y4 + h/2.0*k14);
    k22 = f2(t + h/2.0, *y1 + h/2.0*k11, *y2 + h/2.0*k12, *y3 + h/2.0*k13, *y4 + h/2.0*k14);
    k23 = f3(t + h/2.0, *y1 + h/2.0*k11, *y2 + h/2.0*k12, *y3 + h/2.0*k13, *y4 + h/2.0*k14);
    k24 = f4(t + h/2.0, *y1 + h/2.0*k11, *y2 + h/2.0*k12, *y3 + h/2.0*k13, *y4 + h/2.0*k14);
    
    k31 = f1(t + h/2.0, *y1 + h/2.0*k21, *y2 + h/2.0*k22, *y3 + h/2.0*k23, *y4 + h/2.0*k24);
    k32 = f2(t + h/2.0, *y1 + h/2.0*k21, *y2 + h/2.0*k22, *y3 + h/2.0*k23, *y4 + h/2.0*k24);
    k33 = f3(t + h/2.0, *y1 + h/2.0*k21, *y2 + h/2.0*k22, *y3 + h/2.0*k23, *y4 + h/2.0*k24);
    k34 = f4(t + h/2.0, *y1 + h/2.0*k21, *y2 + h/2.0*k22, *y3 + h/2.0*k23, *y4 + h/2.0*k24);
    
    k41 = f1(t + h, *y1 + h*k31, *y2 + h*k32, *y3 + h*k33, *y4 + h*k34);
    k42 = f2(t + h, *y1 + h*k31, *y2 + h*k32, *y3 + h*k33, *y4 + h*k34);
    k43 = f3(t + h, *y1 + h*k31, *y2 + h*k32, *y3 + h*k33, *y4 + h*k34);
    k44 = f4(t + h, *y1 + h*k31, *y2 + h*k32, *y3 + h*k33, *y4 + h*k34);
...
void convergence_factor_rk4_4(int n1, double h1, double X0,
    ...
    for(int i=1;i<=n1;i++){
        ...
        if (Q4[i] != 0.0) {
            snprintf(s4, sizeof(s4), "%15.6f", Q4[i]);
        } else {
            snprintf(s4, sizeof(s4), "N/A");
        }

        printf("%-10d %-15lf %-15s %-15s %-15s %-15s\n", i, xi, s1, s2, s3, s4);
    }
    ...
    save_in_txt(filename2, Xh, Q2, n1);
    save_in_txt(filename3, Xh, Q3, n1);
    save_in_txt(filename4, Xh, Q4, n1);
