    ...
    switch(choice) {
        ...
        case 4:
            ...
            if(edo_count == 2) {
                ...
                printf("Using pendulum initial conditions:\n");
                printf("Y01 (θ0) = 1.0 rad\n"); 
                printf("Y02 (ω0) = 0.0 rad/τ\n");

                Y1[0] = 1.0;   // θ(0) = 1 rad
                Y2[0] = 0.0;   // ω(0) = 0 rad/τ

                /*
                X0 = 0.0       (tiempo inicial τ)
                Xf = 8.0       (tiempo final - para capturar más de un período)  
                ...
                // Buscar cruces por cero de la velocidad (Y2)
                for(int i = 1; i <= n; i++) {
                    // Detectar cuando Y2 cambia de positiva a negativa (punto de máximo)
                    if(last_y2 > 0 && Y2[i] <= 0) {
                        zero_crossings++;
                        if(zero_crossings == 1) {
                            printf("Primer máximo en τ = %.4f, θ = %.6f\n", X[i], Y1[i]);
                        } else if(zero_crossings == 2) {
                            period = X[i];  // Medio período
                            printf("Segundo máximo en τ = %.4f, θ = %.6f\n", X[i], Y1[i]);
                            break;
                        }
                    }
                    last_y2 = Y2[i];
                ...
                // El período completo es el doble del tiempo entre dos máximos consecutivos
                if(zero_crossings >= 2) {
                    printf("\n===============================================\n");
                    printf("           RESULTADOS PÉNDULO\n");
                    printf("===============================================\n");
                    printf("Período medido: T = %.4f τ\n", period * 2.0);
                    printf("Período pequeñas amplitudes: T = %.4f τ\n", 2.0 * M_PI);
                    printf("Diferencia: ΔT = %.4f τ\n", period * 2.0 - 2.0 * M_PI);
                    printf("===============================================\n");
                } else {
                    printf("No se detectó período completo en el intervalo simulado.\n");
                    printf("Intenta con Xf más grande (ej: 10.0)\n");
                }
