...
// Constantes del problema masa-resorte
#define K 3000.0
#define M 6.0
#define MU 0.5
#define G 9.80665
 ...
 * @return The value of the function at (x, y1, y2)
 */
double f12_masa(double x, double y1, double y2);
 ...
 * @return The value of the function at (x, y1, y2)
 */
double f22_masa(double x, double y1, double y2);


/**
 * Function to define function f2(x, y1, y2) without friction
 * @param x The independent variable
 * @param y1 The first dependent variable
 * @param y2 The second dependent variable
 * @return The value of the function at (x, y1, y2)
 */ 
double f22_masa_sin_friccion(double x, double y1, double y2);
...
int main(int argc, char const *argv[]) {
    ...
    switch(choice) {
        ...
        case 4:
            ...
            if(edo_count == 2) {
                X[0] = X0;
                 int problema_masa_resorte;
                printf("¿Es el problema del sistema masa-resorte con friccion? (1.Si 2.No)\n");
                scanf("%d", &problema_masa_resorte);

                if (problema_masa_resorte == 1) {
                    // ============================================================
                    // == SISTEMA MASA-RESORTE CON FRICCION
                    // ============================================================

                    printf("=============================================\n");
                    printf("    SISTEMA MASA-RESORTE CON FRICCION\n");
                    printf("=============================================\n");

                    // Configurar parámetros específicos
                    double t_final = 2.0; // Tiempo final de simulación
                    h = 0.001; // Paso pequeño para buena precisión
                    n = (int)(t_final / h);
                    ...
                    X[0] = X0;
                    Y1[0] = Y0_MASA; // Posición inicial
                    Y2[0] = 0.0;     // Velocidad inicial (reposo)

                    printf("Constantes del sistema:\n");
                    printf("k = %.1f N/m, m = %.1f kg, μ = %.1f\n", K, M, MU);
                    printf("g = %.5f m/s², y0 = %.3f m\n", G, Y0_MASA);
                    printf("Tiempo final: %.2f s, Paso: %.4f s\n", t_final, h);

                    // Valor teórico del primer pico
                    double y_teorico = Y0_MASA - 4 * MU * M * G / K;
                    printf("Valor teorico del primer pico: y = %.6f m\n", y_teorico);

                    // ============================================================
                    // == SIMULACION CON FRICCION
                    // ============================================================
                    ...
                    for(int i = 0; i < n; i++) {
                        k11 = f12_masa(X[i], Y1[i], Y2[i]);
                        k12 = f22_masa(X[i], Y1[i], Y2[i]);
                        k21 = f12_masa(X[i] + h/2.0, Y1[i] + (h/2.0) * k11, Y2[i] + (h/2.0) * k12);
                        k22 = f22_masa(X[i] + h/2.0, Y1[i] + (h/2.0) * k11, Y2[i] + (h/2.0) * k12);
                        k31 = f12_masa(X[i] + h/2.0, Y1[i] + (h/2.0) * k21, Y2[i] + (h/2.0) * k22);
                        k32 = f22_masa(X[i] + h/2.0, Y1[i] + (h/2.0) * k21, Y2[i] + (h/2.0) * k22);
                        k41 = f12_masa(X[i] + h, Y1[i] + h * k31, Y2[i] + h * k32);
                        k42 = f22_masa(X[i] + h, Y1[i] + h * k31, Y2[i] + h * k32);
                        Y1[i+1] = Y1[i] + (h/6.0) * (k11 + 2.0 * k21 + 2.0 * k31 + k41);
                        Y2[i+1] = Y2[i] + (h/6.0) * (k12 + 2.0 * k22 + 2.0 * k32 + k42);
                        X[i+1] = X[i] + h;
                    }

                    // Buscar el primer pico después del inicial
                    int cruces_por_cero = 0;
                    double primer_pico = Y0_MASA;
                    int indice_primer_pico = 0;
                    ...
                    for (int i = 1; i < n; i++) {
                        ...
                        // Buscar el primer pico después del primer cruce por cero
                        if (cruces_por_cero >= 1) {
                            if (Y1[i] > Y1[i-1] && Y1[i] > Y1[i+1]) {
                                primer_pico = Y1[i];
                                indice_primer_pico = i;
                                break;
                            }
                        }
                    }

                    printf("Primer pico detectado: y = %.6f m en t = %.4f s\n", 
                           primer_pico, X[indice_primer_pico]);
                    printf("Diferencia con valor teorico: %.6f m\n", fabs(primer_pico - y_teorico));
                    printf("Error relativo: %.4f%%\n", fabs(primer_pico - y_teorico) / y_teorico * 100);
                    
                    // ============================================================
                    // == SIMULACION SIN FRICCION
                    // ============================================================
                    ...
                    // Runge-Kutta sin fricción
                    for(int i = 0; i < n; i++) {
                        k11 = f12_masa(X[i], Y1[i], Y2[i]);
                        k12 = f22_masa_sin_friccion(X[i], Y1[i], Y2[i]);
                        k21 = f12_masa(X[i] + h/2.0, Y1[i] + (h/2.0) * k11, Y2[i] + (h/2.0) * k12);
                        k22 = f22_masa_sin_friccion(X[i] + h/2.0, Y1[i] + (h/2.0) * k11, Y2[i] + (h/2.0) * k12);
                        k31 = f12_masa(X[i] + h/2.0, Y1[i] + (h/2.0) * k21, Y2[i] + (h/2.0) * k22);
                        k32 = f22_masa_sin_friccion(X[i] + h/2.0, Y1[i] + (h/2.0) * k21, Y2[i] + (h/2.0) * k22);
                        k41 = f12_masa(X[i] + h, Y1[i] + h * k31, Y2[i] + h * k32);
                        k42 = f22_masa_sin_friccion(X[i] + h, Y1[i] + h * k31, Y2[i] + h * k32);
                        Y1[i+1] = Y1[i] + (h/6.0) * (k11 + 2.0 * k21 + 2.0 * k31 + k41);
                        Y2[i+1] = Y2[i] + (h/6.0) * (k12 + 2.0 * k22 + 2.0 * k32 + k42);
                        X[i+1] = X[i] + h;
                    }

                    // Encontrar amplitud máxima y mínima sin fricción
                    double amplitud_max = Y0_MASA;
                    double amplitud_min = Y0_MASA;
                    for (int i = 0; i <= n; i++) {
                        if (Y1[i] > amplitud_max) amplitud_max = Y1[i];
                        if (Y1[i] < amplitud_min) amplitud_min = Y1[i];
                    }

                    printf("Amplitud maxima sin friccion: %.6f m\n", amplitud_max);
                    printf("Amplitud minima sin friccion: %.6f m\n", amplitud_min);
                    printf("Amplitud inicial: %.6f m\n", Y0_MASA);
                    printf("La amplitud se mantiene constante (como se esperaba)\n");

                    // ============================================================
                    // == GUARDAR RESULTADOS
                    // ============================================================

                    save_in_txt("masa_resorte_con_friccion.txt", X, Y1, n);
                    save_in_txt("masa_resorte_sin_friccion.txt", X, Y1, n);

                    printf("\nDatos guardados en:\n");
                    printf("- masa_resorte_con_friccion.txt\n");
                    printf("- masa_resorte_sin_friccion.txt\n");
                }
            } else if(edo_count == 3) {
                X[0] = X0;
                printf("Insert initial data Y01 = Y1(X0):\n");
...
// Función signo para la fricción
...
// f12 = dy/dt = v
double f12_masa(double X, double Y1, double Y2) {
    return Y2;  // Y1 = posición (y), Y2 = velocidad (v)
}

// f22 = dv/dt = - (k/m) * y - μ * g * sign(v)
double f22_masa(double X, double Y1, double Y2) {
    return -(K/M) * Y1 - MU * G * sign(Y2);
}

// Versión sin fricción
double f22_masa_sin_friccion(double X, double Y1, double Y2) {
    return -(K/M) * Y1;
}
