...
/* For this exercise, we use the secant method.
Incise 1:
Taking a look to the graph f(x) = x * cosh(10 / x) - x - 6, we can see that there is one root:
    Aproximately root is: 9.189, with an error of 0.001
...
So our catenary that passes through the points (+-10, 6) is:
    y = 9.1889 * cosh(a / 9.1889) - 9.1889

Incise 2:
Taking a look to the graph f(x) = x * cosh(12 / x) - x - 5, we can see that there is one root:
    Aproximately root is: 15.167, with an error of 0.000
    Number of iterations: 4
    Function evalutaed in the aproximately root is equal to 0.000
    ✓ Method converges la secante correctly.
      - Converged in 4 iterations (< 10000)
      - f(root) = 0.000 is close to 0
So our catenary that passes through the points (+-12, 5) is:
    y = 15.167 * cosh(a / 15.167) - 15.167
*/
...
double g(double x) {
    // return x * cosh(10 / x) - x - 6; // For Incise 1
