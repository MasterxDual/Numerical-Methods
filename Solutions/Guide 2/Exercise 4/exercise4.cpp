...
/*
Fixed-point method:
    g(x) = ((4 * pow(x,3) - 52 * pow(x,2) - 100) / 160)
    Watching the graph of g(x), we can see that the only root of f(x) = 4 * pow(x,3) - 52 * pow(x,2) + 160 * x - 100 is
    in the straight line (A, B).
    Root is: -0.917, with an error of 0.001
    Number of iterations: 10
    But this is ilogical in terms of the problem. So we'll use Newton-Raphson and Secant methods.

Newton-Raphson method:
    f(x) = 4 * pow(x,3) - 52 * pow(x,2) + 160 * x - 100    
    Watching three roots in the graph of f(x), we can see that:
    Root aproximately is: 0.839, with an error of 0.001
    Number of iterations: 2
    Function evaluated in the aproximately root is equal to 0.000
    ✓ Newton-Raphson method CONVERGE correctly.
      - Converged in two iterations (< 10000)
      - g(root) = 0.000 is near to 0

    Aproximately root is: 3.402, with an error of 0.001
    Number of iterations: 3
    Function evaluated in the aproximately root is equal to -0.000
    ✓ Newton-Raphson method CONVERGE correctly.
      - Converged in three iterations (< 10000)
      - g(root) = -0.000 is near to 0

    Aproximately root is: 8.759, with an error of 0.000
    Number of iterations: 4
    Function evaluated in the aproximately root is equal to 0.000
    ✓ Newton-Raphson method CONVERGE correctly.
      - Converged in four iterations (< 10000)
      - g(root) = 0.000 is near to 0

Secant method:
    Aproximately root is: 0.839, with an error of 0.000
    Number of iterations: 3
    Function evalutaed in the aproximately root is equal to -0.000
    ✓ Method converges la secante correctly.
      - Converged in 3 iterations (< 10000)
      - f(root) = -0.000 is close to 0

    Aproximately root is: 3.402, with an error of 0.000
    Number of iterations: 3
    Function evalutaed in the aproximately root is equal to 0.000
    ✓ Method converges la secante correctly.
      - Converged in 3 iterations (< 10000)
    ...
    Function evalutaed in the aproximately root is equal to -0.000
    ✓ Method converges la secante correctly.
      - Converged in 4 iterations (< 10000)
      - f(root) = -0.000 is close to 0

The result of problem 4 is:
    The side of any of the four squares is root1, root2, or root3 calculated from Newton-Raphson and Secant methods.
*/

// Function prototypes
// For fixed-point method, g(x) and g'(x) are used; for Newton-Raphson and Secant, f(x) and f'(x) are used; see the conceptual difference in a bibliography.
double g(double x);
