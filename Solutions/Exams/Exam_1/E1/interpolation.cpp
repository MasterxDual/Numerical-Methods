...
/**
 * Function to print the symbolic form of the Lagrange basis polynomials
 * @param X Array of X values
 * @param n Number of data points
 */
void print_lagrange_basis_symbolic(double X[], int n);
...
int main(int argc, char const *argv[]) {
    ...
    switch(option) {
        ...
        case 3:
            ...
            printf("------------------Generic Coefficients of Lagrange ------------------\n");
            print_lagrange_basis_symbolic(X, n);
...
void print_lagrange_basis_symbolic(double X[], int n) {
    printf("\n------------------POLINOMIOS BASE DE LAGRANGE (forma simbólica)------------------\n");
    
    for (int k = 0; k < n; k++) {
        printf("L_%d(x) = ", k);
        
        // Numerador: producto de (x - X[i]) para i ≠ k
        printf("[");
        int first_term = 1;
        for (int i = 0; i < n; i++) {
            if (i != k) {
        ...
        // Denominador: producto de (X[k] - X[i]) para i ≠ k
        printf(" / [");
        first_term = 1;
        for (int i = 0; i < n; i++) {
            if (i != k) {
