...
void print_results(double c, double error, int max_iterations, int error_type, double tolerance);
...
int main(int argc, char const *argv[]) {
    ...
    switch(method) {
        case 1:
            ...
            print_results(c, error, max_iterations, error_type, tolerance);
            break;
        case 2:
...
double calculate_error(double c, double old_c, int error_type) {
    if(error_type == 2) {
        ...
        return (fabs((c - old_c)) / (0.5 * fabs(c + old_c))); // Error que pide el ejercicio
...
void print_results(double c, double error, int max_iterations, int error_type, double tolerance) {
    if(error_type == 2) {
        printf("La raíz aproximada es: %0.10lf, con un error porcentual exacto de %0.10lf%%\n", c, error*100);
        printf("Número de iteraciones: %d\n", max_iterations);
        printf("El error estimado es igual a %0.10lf\n", tolerance);
    } else {
        printf("La raíz aproximada es: %0.10lf, con un error de %0.10lf\n", c, error);
        printf("Número de iteraciones: %d\n", max_iterations);
    }
}
