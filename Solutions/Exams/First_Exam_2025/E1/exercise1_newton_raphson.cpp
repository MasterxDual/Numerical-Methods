...
int main(int argc, char const *argv[]) {
    ...
    switch(method) {
        ...
        case 2:
            ...
            // Realizamos un for para calcular en la cuarta iteracion como pide el ejercicio
            for(int i = 0; i < 4; i++) {
                max_iterations++;
...
void show_results(const char* method, double root, double error, int iterations) {
    printf("Aproximately root is: %.10lf, with an estimated porcentual error of %.10lf%%\n", root, error*100);
    printf("Number of iterations: %d\n", iterations);
    if(method != "punto fijo") {
        printf("Function evalutaed in the aproximately root is equal to %.10lf\n", g(root));
    }
}
...
void verify_convergence(const char* method, int iterations, double root, int max_iter) {
    ...
    if(iterations < max_iter) {
        if(fabs(valor_funcion) < 0.01) {
            ...
            printf("  - f(root) = %.10lf is close to 0\n", valor_funcion);
        } else {
            printf("⚠ Method converges in iterations but g(root) = %.10lf is NOT close to 0.\n", valor_funcion);
