...
/* It can't be used Gauss-Seidel because the element a[2][2] is 0. And it's not diagonally dominant at row 1.
You can't do swap of rows because partial-pivoting it's not good for this matrix, and we said that you can't use
partial pivoting in gauss_elimination of this exercise
There's no solution.
Warning: The matrix is not diagonally dominant at row 1.
Error: Zero element on the diagonal at position a[2][2].
The method cannot continue. The matrix has zeros on the diagonal
*/
...
int main(int argc, char const *argv[]) {
    ...
    for(int i = 1; i <= n; i++) {
        for(int j = 1; j <= n; j++) {
            printf("%10.10lf ", a[i][j]);  // 10 total spaces, 6 decimal places
        }
...
void initializeGuess(double Xv[], int n) {
    // Modificamos el Xviejo ya que el ejercicio lo pide
    for(int i = 1; i <= n; i++) {
        Xv[i] = 1.0;
    }
}
...
void printSolution(const char* methodName, double Xn[], int n, int iterations, double error) {
    ...
    printf("The method converged in %d iterations with an error of %10.10lf\n", iterations, error);
}
