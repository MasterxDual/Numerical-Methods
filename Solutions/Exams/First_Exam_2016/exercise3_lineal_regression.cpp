...
int main(int argc, char const *argv[]) {
    ...
    double sumxy, sumx, sumy, distance, sume, sumye;
    ...
    if (!read_data_points("exercise3.txt", X, Y, &n)) {
    ...
    for(int l = 0; l <= 1; l++) {
        // Calculate Independent terms (b[l])
        sumye = 0.0;
        for(int i = 0; i < n; i++) {
            sumye += Y[i] * exp((1 - l) * pow(X[i], 2));
        }
        b[l] = sumye;
        
        // Calculate Matrix A[l][m]
        for(int m = 0; m <= 1; m++) {
            sume = 0.0;
            for(int i = 0; i < n; i++) {
                sume += exp((2-l-m) * pow(X[i], 2));
            }
            A[l][m] = sume;
        }
    }
