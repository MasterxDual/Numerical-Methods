...
/* As it's says, X**(k+1) - X**(k) measures only the change between iterations, while the norm
of A*X**(k) - b measures how well the approximation satisfies the original system. This makes it more reliable. */
...
/** Function to calculate the residual ||AX - b||
 * It's another way to measure the error of the solution
 * It is generally a more appropriate criterion when deciding when to stop the iterative process of the method.
 * @param a Matrix of coefficients
 * @param b Vector of independent terms
 * @param Xn New X = Solution of the matrix
 * @param n Size of the vectors
 * @return The computed residual (Euclidean norm)
 */
double computeResidual(double a[][MAX_SIZE+1], double b[], double Xn[], int n);
...
Matrix to solve:
    ...
    We'll apply Jacobi to solve the system of equations.
    ...
    The method converged in 48 iterations with an error of 0.0

    We'll apply Gauss-Seidel to solve the previous system of equations.
    ...
    The method converged in 14 iterations with an error of   0.0
...
We found the following system of equations to solve:
    ...
    ⚠️  Warning: The matrix is not diagonally dominant at row 1.
    ...
    We'll apply Jacobi to solve the system of equations. 
    The method does not converge, we stop the process.
...
void printSolution(const char* methodName, double Xn[], int n, int iterations, double error) {
    ...
    printf("The method converged in %d iterations with residual norm of %10.6lf\n", iterations, error);
}
...
void jacobiMethod(double a[][MAX_SIZE+1], double b[], double Xv[], double Xn[], int n) {
    ...
    do {
        ...
        new_error = computeResidual(a, b, Xn, n); // Using residual as error
