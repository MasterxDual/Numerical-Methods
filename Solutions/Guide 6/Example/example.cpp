...
/* Inserting the data points, we can see that cubic spline returns this result:
    Cubic Spline Coefficients:
    Spline 1 (from X[0]=3.000 to X[1]=4.500):
      S1(x) = 0.186565x³ + -1.679087x² + 3.617490x + 1.722053
    Spline 2 (from X[1]=4.500 to X[2]=7.000):
      S2(x) = -0.214144x³ + 3.730494x² + -20.725627x + 38.236730
    Spline 3 (from X[2]=7.000 to X[3]=9.000):
      S3(x) = 0.127757x³ + -3.449430x² + 29.533840x + -79.035361
So, if we use an x_hat = 5, we can evaluate it in the spline S2, so we have that S2(5) = 1.15
And we can see in the graph made with gnuplot, that the point (x_hat, S2(x_hat)) = (5, 1,15)
*/
